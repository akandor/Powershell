...
try {
Import-Module -Name ($ScriptDirectory + "\library\Import-Excel")
}
catch {
    ...
    try {
        if($user -ne "" -And $null -ne $user) {
            if($number -ne "" -And $number -ne $null) {
                Set-CsPhoneNumberAssignment -Identity $user -PhoneNumber $number -PhoneNumberType DirectRouting -ErrorAction Stop
            }
            if($cp -ne "" -And $cp -ne $null) {
                Grant-CsTeamsCallingPolicy -Identity $user -PolicyName $cp -ErrorAction Stop
            }
            if($dp -ne "" -And $dp -ne $null) {
                Grant-CsTenantDialPlan -Identity $user -PolicyName $dp -ErrorAction Stop
            }
            if($vrp -ne "" -And $vrp -ne $null) {
                Grant-CsOnlineVoiceRoutingPolicy -Identity $user -PolicyName $vrp -ErrorAction Stop
            }
            if($moh -ne "" -And $moh -ne $null) {
                Grant-CsTeamsCallHoldPolicy -Identity $user -PolicyName $moh -ErrorAction Stop
            }
            if($vm_lang -ne "" -And $vm_lang -ne $null) {
                Set-CsOnlineVoicemailUserSettings -Identity $user -PromptLanguage $vm_lang -ErrorAction Stop
            }
            if($cpp -ne "" -And $cpp -ne $null) {
                Grant-CsTeamsCallParkPolicy -Identity $user -PolicyName $cpp -ErrorAction Stop
            }
            if($cip -ne "" -And $cip -ne $null) {
                Grant-CsCallingLineIdentity -Identity $user -PolicyName $cip -ErrorAction Stop
            }
            if($emp -ne "" -And $emp -ne $null) {
                Grant-CsTeamsEmergencyCallingPolicy -Identity $user -PolicyName $emp -ErrorAction Stop
            }
            Grant-CsTeamsFeedbackPolicy -Identity $user -PolicyName "Disable Survey Policy" -ErrorAction Stop
